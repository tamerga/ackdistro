...
Alibaba ACK Distro is a Kubernetes distribution based on Alibaba Container Service for heterogeneous IaaS environment. Users can get complete content and long-term support for free through Alibaba Cloud. Its components have been verified and security checked by Alibaba Cloud Container Service for Kubernetes (ACK) and Alibaba Group's core business scenarios in large-scale production environment, with industry-leading security and reliability.
​
...
The main difference between ACK Distro and ACK clusters is that ACK clusters are deployed on Alibaba Elastic Compute servers and managed by Alibaba Cloud ACK service, while ACK Distro clusters are managed by yourself and can be deployed on on-premise environments, other cloud service providers or even your own PCs.
​
